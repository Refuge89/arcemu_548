...
class SERVER_DECL Container : public Item
{
	public:
		...
		void SetSlot(uint16 slot, uint64 guid) { SetUInt64Value(CONTAINER_FIELD_SLOT + (slot * 2), guid); }
		uint64 GetSlot(uint16 slot) { return GetUInt64Value(CONTAINER_FIELD_SLOT + (slot * 2)); }
